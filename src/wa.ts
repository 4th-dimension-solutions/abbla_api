...
export async function createSession (options: createSessionOptions) {
  ...
  const handleConnectionClose = async () => {
    ...
    if (code === DisconnectReason.loggedOut || doNotReconnect) {
      (async () => {
        return axios.put(`${process.env.ABBLA_URL}/whatsappapi/${sessionId}`, { status: "DISCONNECTED", qr: null }, {withCredentials: true})
  ...
  socket.ev.on('connection.update', async (update) => {
    ...
    (async () => {
        const sessionStatus = currentSession ? getSessionStatus(currentSession) : "OPENING";
        return axios.put(`${process.env.ABBLA_URL}/whatsappapi/${sessionId}`, { status: sessionStatus , qr: update.qr}, {withCredentials: true}) 
    })().catch((err) => logger.error(err))
    if (connection === 'open') {
...
export async function jidExists(
  ...
  try {
    if (type === 'number') {
      ...
      return {exists: !!result?.exists, resultjid: result?.jid};
    }

    const groupMeta = await session.groupMetadata(jid);
    return {exists: !!groupMeta.id, resultjid: groupMeta.id};
  } catch (e) {
